...
class CarLights {
  ...
  init() {
    const { options, colors, speed, fade } = this;
    ...
    const colorArray = Array.isArray(colors) ? colors.map((c) => new THREE.Color(c)) : [new THREE.Color(colors)];

    for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
      const radius = random(options.carLightsRadius);
      const length = random(options.carLightsLength);
      const speedVal = random(speed);
...
class App {
  constructor(container, options) {
    ...
    this.options = { ...options, distortion: distortions[options.distortion] };
    ...
    this.fog = new THREE.Fog(options.colors.background, options.length * 0.2, options.length * 0.5);
  ...
  initPasses() {
    ...
    this.smaaPass = new EffectPass(this.camera, new SMAAEffect({ preset: 'MEDIUM' }));
...
const Hyperspeed = ({ effectOptions }) => {
  ...
  useEffect(() => {
    ...
    // Initialize App with distortion
    const options = { ...effectOptions, distortion: distortions[effectOptions.distortion] };
  ...
  return <div id="lights" className="w-full h-screen" ref={hyperspeedRef} />;
};
